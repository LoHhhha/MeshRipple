...
def forward_kernel_orig(
    ...
    diff_len = k_len - q_len  # as known as condition len
    ...
    for kh_offset in range(num_k_loop):
        ...
        for j in range(real_q_loop):
            ...
            # k [(diff_len), (q_len)]
            # block_size means top block k
            #   - diff_len -> all can used
            #   - q_len -> causal
            if causal:
                real_topk = tl.sum(
                    tl.where(
                        (topk_idx >= 0)
                        & (topk_idx <= (pid_q_j + diff_len) // block_size),
                        1,
                        0,
                    ),
                    axis=0,
            ...
            for i in range(real_topk):
                ...
                if causal:
                    qk += tl.where(
                        (pid_q_j + diff_len >= c + off_k)[None, :], 0, float("-inf")
                    )
                else:
                    qk += tl.where((c + off_k < k_len)[None, :], 0, float("-inf"))
...
def backward_dkdv(
    ...
    for i in range(0, act_q_len, BLOCK_SIZE_Q):
        ...
        if causal:
            qk += tl.where(
                (idx_q + diff_len)[:, None] >= off_k[None, :], float(0.0), float("-inf")
            )
        else:
            # all can be seen
            qk += tl.where(
                (off_k[None, :] < k_len) & (idx_q[:, None] < q_len),
                float(0.0),
                float("-inf"),
            )
        ...
        ds = ds.to(q.dtype)
        # update dk and dv
        # ds[64,64] q[64,16] p[64,64] do[64,16]
        dk += tl.dot(ds.T, q)
        dv += tl.dot(p.T, do)
    # save dk dv
    ...
    for j in range(real_q_loop):
        ...
        if causal:
            real_topk = tl.sum(
                tl.where(
                    (topk_idx >= 0)
                    & (topk_idx <= (pid_q_j + diff_len) // BLOCK_SIZE_K),
                    1,
                    0,
                ),
                axis=0,
        ...
        for i in range(real_topk):
            ...
            if causal:
                ...
                qk += tl.where((c + off_k)[None, :] < k_len, 0, float("-inf"))
...
def _topk_sparse_attention_fwd(
    ...
    def grid(meta):
        grid = (
            batch_size
            * triton.cdiv(num_k_heads, num_k_loop)
            * triton.cdiv(max_seqlen_q, num_q_loop),
        )
        return grid
...
def topk_sparse_attention(
    ...
    Args:
        ...
        k_cu_seqlens (torch.Tensor): shape [batch_size + 1], similar to cu_seqlens_k in flash_attn_func_varlen.
    ...
    return TopkSparseAttention.apply(
        ...
        (k_cu_seqlens[1:] - k_cu_seqlens[:-1]).max().item(),
        softmax_scale,
        causal,
    )
