...
class Rotary(torch.nn.Module):
    ...
    def forward(self, x, x_cu_seqlen, offset, stride: int = 1):
        ...
        assert positions.size(0) == x.size(0)
...
if __name__ == '__main__':
    net = Rotary(dim=8)
    print(net(torch.randn(3000, 4, 8), torch.Tensor([0, 2, 3000]), offset=torch.Tensor([1, 3]).int(), stride=1).shape)
