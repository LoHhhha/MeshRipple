...
from nsa.ops.utils import is_hopper_gpu
...
def linear_compress_bwd_kernel(
    ...
    x_mask = (
                     (
                             (pid_k * BLOCK_OUTPUT_SEQ_SIZE * KERNEL_STRIDE + off_output_seq_size * KERNEL_STRIDE)[
                                 :, None]
                             + off_kernel_size[None, :]
                     )
                     < x_len
...
class LinearCompress(torch.autograd.Function):
    ...
    def forward(
        ...
        # save for backward
        ctx.save_for_backward(x, w, cu_seqlens, y_cu_seqlens.clone())
        # save value
        ...
        ctx.y_max_seqlen = y_seqlens.max(0)[0].item()
        return y, y_cu_seqlens

    @staticmethod
    def backward(ctx, dy: torch.Tensor, *args) -> Any:
        x, w, cu_seqlens, y_cu_seqlens = ctx.saved_tensors
        ...
        y_max_seqlen = ctx.y_max_seqlen

        total_len, num_heads, head_dim = x.shape
        batch_size = cu_seqlens.shape[0] - 1

        dx = torch.zeros(total_len, num_heads, head_dim, dtype=torch.float32, device=x.device)
        ...
        grid = lambda META: (
            batch_size * num_heads,
            triton.cdiv(y_max_seqlen, META["BLOCK_OUTPUT_SEQ_SIZE"]),
