...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cu_seqlens: torch.Tensor, start: int = 0, stride: int = 1, interleaved=False):
        ...
        x = apply_rotary_emb(x, cos, sin, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen, interleaved=interleaved)
        return x

    def forward_dev(self, x: torch.Tensor, cu_seqlens: torch.Tensor, start: torch.Tensor, stride: int = 1,
                    interleaved=False):
        total_seqlen, num_heads, head_dim = x.shape
        seqlens = cu_seqlens[1:] - cu_seqlens[:-1]
        assert len(seqlens) == len(start)

        y = torch.zeros_like(x)
        for idx in range(len(seqlens)):
            max_position_ids = torch.arange(
                start[idx].item(), seqlens[idx].item() * stride + start[idx].item(), stride, device=x.device
            )[:seqlens[idx].item()]
            cos, sin = self.generate_cos_sin(x, max_position_ids[None, ...])
            cos, sin = cos.squeeze(0), sin.squeeze(0)
            y[cu_seqlens[idx]:cu_seqlens[idx + 1]] = apply_rotary_emb(
                x[cu_seqlens[idx]:cu_seqlens[idx + 1]],
                cos,
                sin,
                cu_seqlens=torch.Tensor([0, seqlens[idx].item()]).long().to(x.device),
                max_seqlen=seqlens[idx].item(),
