...
# modify:
#   1. support k_len >= q_len
#   2. causal: k = stack(token(k_len-q_len), token(q_len))
#       token(k_len-q_len) can be see in all q, token(q_len) just like prev
...
from nsa.ops.utils import get_num_warps_stages, is_hopper_gpu
...
def forward_kernel(
    ...
    act_k_start = tl.load(origin_cu_seqlens_k + pid_b)
    act_k_len = tl.load(origin_cu_seqlens_k + pid_b + 1) - act_k_start
    diff_len = act_k_len - q_len
    # where to start to calculate q
    if causal:
        # act_k_len - q_len means how many tokens in kv can be seen by q additionally
        q_start_in_seq = pid_q * BLOCK_SIZE_Q + max(kernel_size - 1 - diff_len, 0)
    else:
        # all k can be seen
        q_start_in_seq = pid_q * BLOCK_SIZE_Q + max(kernel_size - 1 - act_k_len, 0)
    ...
    # attention
    # from k's [lo, hi) to compute qk and update acc_o
    lo = 0
    if causal:
        # the final q(q_start_in_seq + BLOCK_SIZE_Q - 1) can see k in range [0, q_start_in_seq + BLOCK_SIZE_Q - kernel_size + diff_len)
        hi = min(
            k_len,
            (q_start_in_seq + BLOCK_SIZE_Q - kernel_size + diff_len) // kernel_stride
            + 1,
        )
    else:
        # all compressed_k can be seen
        hi = k_len
    ...
    for i in range(lo, hi, BLOCK_SIZE_K):
        ...
        if causal:
            # diff_len can be seen for all q
            # here, i means compressed_k's idx
            #   compressed_k[i] come from [i * kernel_stride, i * kernel_stride + kernel_size - 1]
            #   off_k is tl.arange(0, BLOCK_SIZE_K) * kernel_stride + kernel_size - 1
            # so act_k_idx is i * kernel_stride + off_k
            # off_q is tl.arange(0, BLOCK_SIZE_Q) + q_start_in_seq
            #   q_start_in_seq means where we start to calculate q
            # so q_idx is off_q
            mask = (off_q + diff_len)[:, None] >= (i * kernel_stride + off_k)[None, :]
        else:
            # all k can be seen, but need to mask the padding part
            mask = ((i * kernel_stride + off_k)[None, :] < act_k_len) & (
                off_q[:, None] < q_len
            )
...
def backward_dkdv(
    ...
    if causal:
        # the first ck(pid_k * BLOCK_SIZE_K) contains k[pid_k * BLOCK_SIZE_K * kernel_stride : pid_k * BLOCK_SIZE_K * kernel_stride + kernel_size - 1]
        # the pid_k * BLOCK_SIZE_K * kernel_stride + kernel_size - 1 can be seen by pid_k * BLOCK_SIZE_K * kernel_stride + kernel_size - 1 - diff_len
        q_lo = tl.maximum(
            pid_k * BLOCK_SIZE_K * kernel_stride + kernel_size - 1 - diff_len,
            0,
        )
    else:
        q_lo = 0
    ...
    for i in range(q_lo, q_len, BLOCK_SIZE_Q):
        ...
        if causal:
            mask = off_k[:, None] <= (off_q + i + diff_len)[None, :]
        else:
            # all k can be seen, but need to mask the padding part
            mask = (off_k[:, None] < act_k_len) & ((off_q + i)[None, :] < q_len)

        qk = tl.where(mask, float(0.0), float("-inf"))
...
def _compressed_attention_fwd(
    ...
    origin_cu_seqlens_k: torch.Tensor,
    ...
    assert (
        k_len == v_len
    )  # Removed q_len > k_len assertion to support cases where k_len > q_len
    # gqa
...
class CompressedAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(
            q, k, v, o, lse, cu_seqlens_q, cu_seqlens_k, origin_cu_seqlens_k
        )
...
def score_kernel(
    ...
    # init score and causal_mask
    s = tl.zeros((BLOCK_SIZE_Q, BLOCK_SIZE_K), dtype=tl.float32)

    if causal:
        causal_mask = (off_q + diff_len)[:, None] >= (
            off_k * kernel_stride + kernel_size - 1
        )[None, :]
    else:
        causal_mask = (
            (off_k * kernel_stride + kernel_size - 1)[None, :] < act_k_len
        ) & (off_q[:, None] < q_len)
    ...
    # causal_mask_ptrs = tl.make_block_ptr(
    #     base=causal_mask_ptr + pid_kh * stride_sh + q_start * stride_sq,
    #     shape=(q_len, k_len),
    #     strides=(stride_sq, stride_sk),
    #     offsets=(pid_q * BLOCK_SIZE_Q, pid_k * BLOCK_SIZE_K),
    #     block_shape=(BLOCK_SIZE_Q, BLOCK_SIZE_K),
    #     order=(1, 0),
    # )
    # tl.store(causal_mask_ptrs, causal_mask.to(causal_mask_ptr.dtype.element_ty), boundary_check=(0, 1))
...
def _get_attention_score(
    ...
    # init score and causal
    score = torch.zeros(
        num_k_heads, q_len, max_seqlen_k, dtype=torch.float32, device=q.device
    )
    # causal_mask = torch.zeros_like(score, dtype=torch.int8)
...
def _transform_score_kernel(
    ...
    off_bq = (off_q + act_k_len - q_len) // block_size
...
def transform_score(
    ...
    max_act_seqlen_k = (origin_cu_seqlens_k[1:] - origin_cu_seqlens_k[:-1]).max().item()
    max_blocks = math.ceil(max(max_seqlen_q, max_act_seqlen_k) / block_size)
...
def compressed_attention(
    ...
    Args:
        ...
        k (torch.Tensor): shape [total_kv_len, num_kv_heads, head_dim], compressed
        v (torch.Tensor): shape [total_kv_len, num_kv_heads, head_dim], compressed
        ...
        origin_cu_seqlens_k (torch.Tensor): shape [batch_size + 1], similar to cu_seqlens_k in flash_attn_func_varlen, uncompressed
        ...
        causal: (bool, optional): Whether causal attention should be computed. Defaults to True.
    ...
    with torch.no_grad():
        ...
        q_idx = torch.cat(
            [
                torch.arange(
                    cu_seqlens_q[i + 1].item() - cu_seqlens_q[i].item(), device=q.device
                )
        ...
        if causal:
            # for each q, it can see k[0:act_k_len-q_len+i]
            q_idx += torch.cat(
                [
                    torch.full(
                        (cu_seqlens_q[i + 1].item() - cu_seqlens_q[i].item(),),
                        (
                            origin_cu_seqlens_k[i + 1].item()
                            - origin_cu_seqlens_k[i].item()
                            - (cu_seqlens_q[i + 1].item() - cu_seqlens_q[i].item())
                        ),
                        device=q.device,
                    )
            ...
            # ensure all can be seen
            q_idx[:] = int(0x3F3F3F3F)
        ...
        if parallel_topk_compute:
            ...
            head_tile = 1
            assert (
                num_k_heads % head_tile == 0
            ), f"Num kv heads: {num_k_heads}, head_tile: {head_tile}"
            for h in range(num_k_heads // head_tile):
                ...
                topk_idx = topk_idx.sort(dim=-1, stable=True).values
