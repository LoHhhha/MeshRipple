...
class Rotary(nn.Module):
    ...
    def forward(self, x_BTHD, offset=0):
        ...
        assert torch.max(offset + seq_len) <= self.max_seq_len, f"Sequence length {offset + seq_len} exceeds maximum length {self.max_seq_len}"
...
class MultiheadCrossFlashrope(nn.Module):
    ...
    def forward(self, x, start_pos, context, attention_mask=None, causal=False):
        """
        Args:
            x: [batch_size, tgt_len, embed_dim]
            start_pos: [batch_size]
        ...
        if self.kv_cache_enabled:
            ...
            if self.k_cache is not None:
                ...
                q_len = q.size(1)
                if not torch.all(start_pos == start_pos[0]):
                    raise AssertionError(f"start_pos elements are not all equal: {start_pos}")
